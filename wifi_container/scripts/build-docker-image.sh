...
DEFAULT_IMAGE=csa-certification-tool-wifi
...
[[ ${*/--multiarch//} != "${*}" ]] && {
    docker buildx build --push --platform linux/amd64,linux/arm64 -t "$GHCR_ORG"/"$ORG"/"$IMAGE":"$VERSION" .
    exit 0
}

# Don't run `docker build` and `docker image prune` when `--skip-build` in arguments
[[ ${*/--skip-build//} != "${*}" ]] || {
    docker build "${BUILD_ARGS[@]}" -t "$GHCR_ORG"/"$ORG"/"$IMAGE":"$VERSION" .
