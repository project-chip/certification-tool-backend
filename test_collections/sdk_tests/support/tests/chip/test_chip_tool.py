...
from test_collections.sdk_tests.support.chip.chip_tool import (
    CHIP_APP_EXE,
...
async def test_start_chip_server_waiting_failure() -> None:
    ...
    # clean up:
    chip_tool._ChipTool__server_logs = None
    chip_tool._ChipTool__chip_server_id = None
...
async def test_run_test_default_config() -> None:
    ...
    chip_tool._ChipTool__test_harness_runner = runner
    chip_tool._ChipTool__server_started = True

    with mock.patch.object(
        target=chip_tool, attribute="start_runner"
    ), mock.patch.object(
        target=runner,
        attribute="run",
        return_value=True,
    ...
    # clean up:
    chip_tool._ChipTool__test_harness_runner = None
    chip_tool._ChipTool__server_started = False
