...
def custom_python_test_collection(
    ...
    """
    Create custom Python test collection.

    Note: Test file generation is handled by test_manager.py during initialization.
    This function only creates collections from existing test files.

    Args:
        python_test_folder: Folder containing custom Python test scripts
        tests_file_path: Path to the parsed test file

    Returns:
        Custom Python test collection or None if no tests found
    """
    return _create_custom_python_test_collection(python_test_folder, tests_file_path)


def _create_custom_python_test_collection(
    python_test_folder: SDKTestFolder = CUSTOM_PYTHON_SCRIPTS_FOLDER,
    tests_file_path: Path = CUSTOM_PYTHON_TESTS_PARSED_FILE,
) -> Optional[PythonCollectionDeclaration]:
    """
    Create custom Python test collection from pre-generated test files.

    This function assumes test files have already been generated by the test manager
    and does NOT trigger test file generation itself.
    ...
    Returns:
        Custom Python test collection or None if no valid suites found
    """
    collection = PythonCollectionDeclaration(
