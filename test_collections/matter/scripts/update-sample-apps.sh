...
if [[ -z "$DOCKER_IMAGE_FOUND" && "$HOST_ARCH" != "aarch64" ]]; then
    ...
    echo "available locally for this architecture ($HOST_ARCH)."
    echo ""
    echo "Once the installer completes:"
    echo "  1. Build the SDK image locally (takes about an hour):"
    echo "     $MATTER_PROGRAM_DIR/scripts/build-local-sdk-image.sh"
    echo "  2. Install the sample apps:"
